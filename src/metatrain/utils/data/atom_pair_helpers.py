...
def get_bidirectional_edges(tmap: TensorMap) -> TensorMap:
    ...
    """
    is_atomic_basis = "first_atom_type" in tmap.keys.names
    is_coupled = "o3_lambda" in tmap.keys.names
    ...
    for key, block in tmap.items():
        ...
        # Get the components of the reverse block.
        if is_coupled:
            reverse_components = block.components
        else:
            reverse_components = [
                Labels(
                    names=block.components[0].names, values=block.components[1].values
                ),
                Labels(
                    names=block.components[1].names, values=block.components[0].values
                ),
            ]

        if not is_coupled and key["first_atom_type"] == key["second_atom_type"]:
            # If this is a block for edges between atoms of the same type,
            # find the block containing the reverse direction
            search_key = {name: key[name] for name in key.names}
            search_key["o3_lambda_1"], search_key["o3_lambda_2"] = (
                search_key["o3_lambda_2"],
                search_key["o3_lambda_1"],
            )
            reverse_block = tmap.block(search_key)
        else:
            reverse_block = block

        # Get the properties of the reverse block.
        properties = reverse_block.properties.values.clone()
        # Swap n_1 with n_2
        properties[:, [i_n1, i_n2]] = properties[:, [i_n2, i_n1]]
        if is_coupled:
            ...
            properties = properties[properties_order]
        ...
        reverse_values = reverse_block.values
        if is_coupled:
            # If o3_sigma is -1, the reverse block should have its values negated.
            reverse_values = reverse_values * key["o3_sigma"]
        else:
            # We need to transpose the values and properties
            reverse_values = reverse_values.transpose(1, 2)
            reverse_values = reverse_values.index_select(-1, properties_order)
        ...
        if is_atomic_basis and key["first_atom_type"] < key["second_atom_type"]:
            ...
            # Block between atoms of the same type.
            selection = block.properties.select(reverse_properties)
            reverse_values = reverse_values.index_select(-1, selection)

            new_block = TensorBlock(
                values=torch.cat([block.values, reverse_values], dim=0),
                samples=Labels(
                    names=block.samples.names,
        ...
        new_blocks.append(new_block)
        if not is_coupled and key["first_atom_type"] != key["second_atom_type"]:
            # Swap o3_lambda_1 with o3_lambda_2
            new_key[[0, 1]] = new_key[[1, 0]]
